...
public class GSKDatabase extends SQLiteOpenHelper {
    ...
    public static final String DATABASE_NAME = "LOREAL_PROMOTER_Db1";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(TableBean.getMapping_menuTable());
        ...
        db.execSQL(CommonString.CREATE_TABLE_SHARE_OF_SHELF_POPUP_DATA);
    ...
    public void insertJCPData(JourneyPlanGetterSetter data) {
        ...
        try {
            for (int i = 0; i < data.getStore_cd().size(); i++) {
                ...
                values.put("CHANNEL", data.getChannel().get(i));
                values.put("FLOOR_STATUS", data.getFloor_status().get(i));
                values.put("BACKROOK_STATUS", data.getBackroom_status().get(i));
                values.put("REGION_CD", data.getRegion_Id().get(i));
    ...
    public void insertfocusPerformanceData(FocusPerformanceGetterSetter data) {
        ...
        try {
            for (int i = 0; i < data.getSTORE_CD().size(); i++) {
                ...
                values.put("SALES", Float.valueOf(data.getSALES().get(i)));
                values.put("ALL_TARGET", Float.valueOf(data.getALL_TARGET().get(i)));
                values.put("SHOW_TARGET", Float.valueOf(data.getSHOW_TARGET().get(i)));
    ...
    public ArrayList<SamplingMasterGetterSetter> getSamplingData(String store_cd) {
        ...
        try {
            ...
            if (dbcursor != null) {
                ...
                while (!dbcursor.isAfterLast()) {
                    ...
                    sb.setChannel_cd((dbcursor.getString(dbcursor.getColumnIndexOrThrow("CHANNEL_CD"))));
                    sb.setChannel((dbcursor.getString(dbcursor.getColumnIndexOrThrow("CHANNEL"))));
                    sb.setFloor_status((dbcursor.getString(dbcursor.getColumnIndexOrThrow("FLOOR_STATUS"))));
                    sb.setBackroom_status((dbcursor.getString(dbcursor.getColumnIndexOrThrow("BACKROOK_STATUS"))));
                    sb.setRegion_Id((dbcursor.getString(dbcursor.getColumnIndexOrThrow("REGION_CD"))));
    ...
    public ArrayList<StockNewGetterSetter> getStockAvailabilityDataNew(String channel_cd) {
        Log.d("Fetching", "Storedata--------------->Start<------------");
        ArrayList<StockNewGetterSetter> list = new ArrayList<StockNewGetterSetter>();
        Cursor dbcursor = null;
        ...
        try {
            ...
            if (dbcursor != null) {
                ...
                while (!dbcursor.isAfterLast()) {
                    StockNewGetterSetter sb = new StockNewGetterSetter();
                    sb.setCategory_cd(dbcursor.getString(dbcursor.getColumnIndexOrThrow("BRAND_CD")));
                    sb.setCategory(dbcursor.getString(dbcursor.getColumnIndexOrThrow("BRAND")));
                    sb.setHimalaya_camera(dbcursor.getString(dbcursor.getColumnIndexOrThrow("HIMALAYA_PHOTO")));
                    sb.setCategory_camera(dbcursor.getString(dbcursor.getColumnIndexOrThrow("CATEGORY_PHOTO")));
    ...
    public ArrayList<StockNewGetterSetter> getmappingStockDataNew(String account_cd, String city_cd, String storetype_cd) {
        Log.d("Fetching", "Storedata--------------->Start<------------");
        ArrayList<StockNewGetterSetter> list = new ArrayList<StockNewGetterSetter>();
        Cursor dbcursor = null;
    ...
    public ArrayList<StockNewGetterSetter> getmappingStockInData() {
        ...
        try {
            ...
            dbcursor = db.rawQuery("SELECT BR.BRAND_CD, SB.SUB_CATEGORY||'-'||BR.BRAND as BRAND,SB.CATEGORY_CD FROM BRAND_MASTER BR " +
    ...
    public ArrayList<StockNewGetterSetter> getStockBackOfficeSkuDataNew(String channel_cd, String categord_cd) {
        Log.d("Fetching", "Storedata--------------->Start<------------");
        ArrayList<StockNewGetterSetter> list = new ArrayList<StockNewGetterSetter>();
        Cursor dbcursor = null;
        ...
        try {
            ...
            dbcursor = db.rawQuery("SELECT DISTINCT SD.SKU_CD, SD.SKU,SD.BRAND_CD,SD.BRAND ,BM.COMPANY_CD " +
                    "FROM MAPPING_CHANNEL_SKU CD " +
                    "INNER JOIN SKU_MASTER SD " +
                    "ON CD.SKU_CD = SD.SKU_CD " +
                    "INNER JOIN BRAND_MASTER BM " +
                    "ON BM.BRAND_CD = SD.BRAND_CD " +
                    "WHERE CD.CHANNEL_CD ='" + channel_cd + "' AND  BM.BRAND_CD ='" + categord_cd + "' " +
    ...
    public ArrayList<StockNewGetterSetter> getStockSkuDataNew(String account_cd, String city_cd, String storetype_cd, String categord_cd) {
        Log.d("Fetching", "Storedata--------------->Start<------------");
        ArrayList<StockNewGetterSetter> list = new ArrayList<StockNewGetterSetter>();
        Cursor dbcursor = null;
    ...
    public ArrayList<StockNewGetterSetter> getPromotionBrandData1(String account_cd, String city_cd, String storetype_cd) {
        ...
        try {
            ...
            dbcursor = db.rawQuery("SELECT DISTINCT SD.BRAND_CD, SD.BRAND, CA.CATEGORY_CD " +
                    "FROM MAPPING_PROMOTION CD " +
                    "INNER JOIN BRAND_MASTER SD ON CD.BRAND_CD = SD.BRAND_CD " +
                    "INNER JOIN SUB_CATEGORY_MASTER SB ON SD.SUB_CATEGORY_CD = SB.SUB_CATEGORY_CD " +
                    "INNER JOIN CATEGORY_MASTER CA ON SB.CATEGORY_CD = CA.CATEGORY_CD " +
                    "INNER JOIN MAPPING_USER_CATEGORY MC ON CA.CATEGORY_CD = MC.CATEGORY_CD " +
                    "WHERE CD.KEYACCOUNT_CD ='" + account_cd + "' AND CITY_CD  ='" + city_cd + "' AND STORETYPE_CD  ='" + storetype_cd + "'" +
                    "ORDER BY SD.BRAND_SEQUENCE", null);
    ...
    public ArrayList<AssetInsertdataGetterSetter> getAssetCategoryData(String store_cd) {
        ...
        try {
            ...
            dbcursor = db.rawQuery("SELECT * FROM(" +
                    " SELECT DISTINCT CATEGORY_CD, CATEGORY" +
                    " FROM CATEGORY_MASTER BD WHERE CATEGORY_CD IN( SELECT DISTINCT CATEGORY_CD FROM MAPPING_ASSET WHERE STORE_CD ='" + store_cd + "' )" +
                    " )AS T1" +
                    " INNER JOIN MAPPING_USER_CATEGORY MC ON T1.CATEGORY_CD = MC.CATEGORY_CD", null);
    ...
    public long InsertOpeningStockBackOfficelistData(String storeid,
                                                     HashMap<StockNewGetterSetter, List<StockNewGetterSetter>> data, List<StockNewGetterSetter> save_listDataHeader) {
        ContentValues values = new ContentValues();
        ContentValues values1 = new ContentValues();
        long l2 = 0;
        try {
            ...
            for (int i = 0; i < save_listDataHeader.size(); i++) {
                ...
                for (int j = 0; j < data.get(save_listDataHeader.get(i)).size(); j++) {
                 ...
                 l2=   db.insert(CommonString.TABLE_STOCK_BACKOFFICE_DATA, null, values1);
                }
            }
            db.setTransactionSuccessful();
            db.endTransaction();
    ...
    public ArrayList<PromotionInsertDataGetterSetter> getPromotionDataFromDatabase(String storeId, String brand_cd) {
        ...
        try {
            dbcursor = db.rawQuery("SELECT SD.PID, SD.CAMERA, SD.PROMOTION,SD.PRESENT_SPIN,SD.PROMO_STOCK, SD.RUNNING_POS,SD.REMARK,SD.REMARK_CD," +
            ...
            if (dbcursor != null) {
                ...
                while (!dbcursor.isAfterLast()) {
                    ...
                    String promoStock = dbcursor.getString(dbcursor.getColumnIndexOrThrow("PROMO_STOCK"));
                    sb.setPromoStock(promoStock);

                    String runningPos = dbcursor.getString(dbcursor.getColumnIndexOrThrow("RUNNING_POS"));
                    sb.setRunningPOS(runningPos);
...
//Get Promotion Database Data
    ...
    public ArrayList<StockNewGetterSetter> getOpeningStockDataFromDatabase(String store_cd, String categord_cd) {
        ...
        try {
            dbcursor = db.rawQuery("SELECT SD.STOCK_1,SD.STOCK_2,SD.STOCK_3, " +
    ...
    public boolean isStockBackRoomDataAllFilled(String storeId) {
        ...
        try {
            ...
            if (dbcursor != null) {
                dbcursor.moveToFirst();
                while (!dbcursor.isAfterLast()) {
                    if (dbcursor.getString(dbcursor.getColumnIndexOrThrow("STOCK_1")) == null || dbcursor.getString(dbcursor.getColumnIndexOrThrow("STOCK_1")).equals("")) {
                        filled = false;
                        break;
                    } else {
                        filled = true;
    ...
    public ArrayList<StockNewGetterSetter> getClosingStockDataFromDatabase(String account_cd, String category_cd, String store_cd) {
        ...
        try {
            ...
            dbcursor = db.rawQuery("SELECT DISTINCT SD.SKU_CD AS SKU_CD, SD.SKU AS SKU,SD.BRAND_CD AS BRAND_CD,SD.BRAND AS BRAND, S.STOCK_1" +
                    " AS OPENING_STOCK, S.MIDDAY_STOCK AS MIDDAY_STOCK, S.CLOSING_STOCK AS CLOSING_STOCK_FLOOR " +
                    " FROM MAPPING_STOCK CD " +
    ...
    public ArrayList<StockNewGetterSetter> getMiddayStockDataFromDatabase(String store_cd, String categord_cd) {
        ...
        try {
            ...
            dbcursor = db.rawQuery("SELECT SD.STOCK_1,SD.STOCK_2,SD.STOCK_3,SD.MIDDAY_STOCK, " +
                    "SD.SKU_CD,SD.COMPANY_CD, SD.SKU, SM.BRAND_CD, SM.BRAND, " +
                    " SD.OPENING_FACING ,SD.STOCK_UNDER_DAYS " +
                    "FROM openingHeader_data CD " +
                    "INNER JOIN STOCK_DATA SD " +
    ...
    public boolean isMiddayDataFilled(String storeId) {
        ...
        try {
            ...
            dbcursor = db.rawQuery("SELECT MIDDAY_STOCK FROM STOCK_DATA " + "WHERE STORE_CD= '" + storeId + "'", null);
    ...
    public void insertMappingUserCategoryData(MappingUserCategoryGetterSetter data) {
        db.delete("MAPPING_USER_CATEGORY", null, null);
        ContentValues values = new ContentValues();
        ...
        try {
            ...
            for (int i = 0; i < data.getCategory_cd().size(); i++) {
                values.put("CATEGORY_CD", Integer.parseInt(data.getCategory_cd().get(i)));
                db.insert("MAPPING_USER_CATEGORY", null, values);

            }
        } catch (Exception ex) {
            Log.d("Database Exception while Insert MAPPING_USER_CATEGORY Data ",
                    ex.toString());
        }
    ...
    public void insertQuestionAnsData(TodayQuestionGetterSetter data) {
        db.delete("PERFORMANCE_OQAD", null, null);
        ContentValues values = new ContentValues();

        try {

            for (int i = 0; i < data.getTOT_QUESTION().size(); i++) {
                values.put("TOT_QUESTION", Integer.parseInt(data.getTOT_QUESTION().get(i)));
                values.put("RIGHT_ANSWER", Integer.parseInt(data.getRIGHT_ANSWER().get(i)));
                db.insert("PERFORMANCE_OQAD", null, values);

            }
        } catch (Exception ex) {
            Log.d("Database PERFORMANCE_OQADData ",
                    ex.toString());
        }
    ...
    public ArrayList<PromotionInsertDataGetterSetter> getPromotionUploadData(String storeId) {
        ...
        Cursor dbcursor = null;
        try {
            dbcursor = db.rawQuery("SELECT SD.PID, SD.PRESENT_SPIN,SD.REMARK_CD,SD.CAMERA, SD.PROMOTION,SD.PROMO_STOCK, SD.RUNNING_POS,SD.REMARK, CD.BRAND_CD,CD.BRAND FROM openingHeader_Promotion_data CD INNER JOIN PROMOTION_DATA SD " +
    ...
    public void removeall_sampled_Data(String store_cd) {
        db.delete("SAMPLED_DATA", CommonString.KEY_STORE_CD + "='" + store_cd + "'", null);
    }
    ...
    public ArrayList<ShareOfShelfGetterSetter> getHeaderShareOfSelfImageData(String store_cd) {
        ...
        try {
            ...
            dbcursor = db.rawQuery("SELECT distinct SB.SUB_CATEGORY_CD AS SUB_CATEGORY_CD, CA.CATEGORY||'-'||SB.SUB_CATEGORY AS SUB_CATEGORY, IFNULL(D.IMAGE_CAT_FACING,'') AS IMAGE_CAT_FACING, IFNULL(D.CAT_FACING,'')" +
    ...
    public ArrayList<ShareOfShelfGetterSetter> getShareofShelfBrandData(String categord_cd, String store_cd) {
        ...
        try {
            ...
            dbcursor = db.rawQuery("SELECT BR.BRAND_CD, BR.BRAND AS BRAND, IFNULL(D.FACING,'') " +
                    ...
                    "AS D ON BR.SUB_CATEGORY_CD = D.SUB_CATEGORY_CD AND BR.BRAND_CD = D.BRAND_CD AND BR.COMPANY_CD='1'", null);
    ...
    public ArrayList<StockNewGetterSetter> getMiddayStockInDatabase(String store_cd) {
        Log.d("Fetching", "Mid Stock data--------------->Start<------------");
        ArrayList<StockNewGetterSetter> list = new ArrayList<StockNewGetterSetter>();
        Cursor dbcursor = null;
        ...
        try {
            ...
            if (dbcursor != null) {
                ...
                while (!dbcursor.isAfterLast()) {
                    ...
                    pgs.setSHOW_TARGET(dbcursor.getString(dbcursor.getColumnIndexOrThrow("SHOW_TARGET")));
    ...
    public ArrayList<TodayQuestionGetterSetter> getTodayQuestionAnsData() {

        Log.d("Fetching facing competitor--------------->Start<------------",
                "------------------");
        ArrayList<TodayQuestionGetterSetter> list = new ArrayList<TodayQuestionGetterSetter>();
        Cursor dbcursor = null;

        try {
            dbcursor = db.rawQuery("SELECT * from PERFORMANCE_OQAD"
                    , null);

            if (dbcursor != null) {
                dbcursor.moveToFirst();
                while (!dbcursor.isAfterLast()) {
                    TodayQuestionGetterSetter pgs = new TodayQuestionGetterSetter();

                    pgs.setTOT_QUESTION(dbcursor.getString(dbcursor.getColumnIndexOrThrow("TOT_QUESTION")));
                    pgs.setRIGHT_ANSWER(dbcursor.getString(dbcursor.getColumnIndexOrThrow("RIGHT_ANSWER")));
    ...
    public long InsertPopupStockinData(StoreStockinPopupGetterSetter dateset, String storeid, String visitdate) {
        db.delete(CommonString.TABLE_SHARE_OF_SHELF_POPUP_DATA, CommonString.KEY_STORE_ID + "='" + storeid + "'", null);
        ContentValues values = new ContentValues();
        long id = 0;
        try {
            values.put("STORE_ID", storeid);
            values.put("VISIT_DATE", visitdate);
            values.put("CURRENT_DATE", dateset.getCurrent_date());

            id = db.insert(CommonString.TABLE_SHARE_OF_SHELF_POPUP_DATA, null, values);
            if (id > 0) {
                return id;
            } else {
                return 0;
    ...
    public StoreStockinPopupGetterSetter getStockInPopupData(String store_id) {
        Log.d("FetchinggetCityMasterData--------------->Start<------------",
                "------------------");
        StoreStockinPopupGetterSetter sb = new StoreStockinPopupGetterSetter();
        Cursor dbcursor = null;

        try {

            dbcursor = db.rawQuery("SELECT * FROM " + CommonString.TABLE_SHARE_OF_SHELF_POPUP_DATA + " WHERE " + CommonString.KEY_STORE_ID + " = " + store_id, null);
            ...
            if (dbcursor != null) {
                ...
                while (!dbcursor.isAfterLast()) {
                    ...
                    sb.setKey_id(dbcursor.getString(dbcursor.getColumnIndexOrThrow("KEY_ID")));
                    sb.setCurrent_date(dbcursor.getString(dbcursor.getColumnIndexOrThrow("CURRENT_DATE")));
    ...
    public StoreStockinPopupGetterSetter getStockInPopupuPLOADData(String store_id) {
        Log.d("FetchinggetCityMasterData--------------->Start<------------",
                "------------------");
        StoreStockinPopupGetterSetter sb = new StoreStockinPopupGetterSetter();
        Cursor dbcursor = null;
    ...
    public void deleteStockinbilldate(String storeid) {
        db.delete(CommonString.TABLE_SHARE_OF_SHELF_POPUP_DATA, CommonString.KEY_STORE_ID + "='" + storeid + "'", null);
    }
    ...
    public void insertMappingChannelData(MappingChannalSkuGetterSetter data) {
        db.delete("MAPPING_CHANNEL_SKU", null, null);
        ContentValues values = new ContentValues();

        try {

            for (int i = 0; i < data.getChannel_cd().size(); i++) {
                values.put("CHANNEL_CD", Integer.parseInt(data.getChannel_cd().get(i)));
                values.put("SKU_CD", Integer.parseInt(data.getChannel().get(i)));
                db.insert("MAPPING_CHANNEL_SKU", null, values);

            }
        } catch (Exception ex) {
            Log.d("Database MAPPING_CHANNEL_SKU ",
                    ex.toString());
        }
    ...
    public void mappingmenuenentry(MappingMenuOptionGetterSetter data) {
        db.delete("MAPPING_MENU_OPTION", null, null);
        ContentValues values = new ContentValues();

        try {

            for (int i = 0; i < data.getRegion_Id().size(); i++) {
                values.put("REGION_CD", Integer.parseInt(data.getRegion_Id().get(i)));
                values.put("MENU_NAME", data.getMenu_name().get(i));
                values.put("MENU_ICON", data.getMenu_icon().get(i));
                values.put("MENU_ICON_DONE", data.getMenu_icon_done().get(i));
                values.put("IMAGE_PATH", data.getImage_path().get(i));
                values.put("MENU_ICON_GREY", data.getMenu_icon_gray().get(i));
                values.put("SEQUENCE", data.getSEQUENCE().get(i));

                db.insert("MAPPING_MENU_OPTION", null, values);

            }
        } catch (Exception ex) {
            Log.d("Database MAPPING_MENU_OPTION ", ex.toString());
        }

    }

    public ArrayList<MappingMenuOptionGetterSetter> getmappingMenuEntryByRegion(String region_Id) {

        ArrayList<MappingMenuOptionGetterSetter> list = new ArrayList<>();
        Cursor dbcursor = null;

        try {
            dbcursor = db.rawQuery("SELECT distinct REGION_CD,MENU_NAME,MENU_ICON,MENU_ICON_DONE,MENU_ICON_GREY,IMAGE_PATH from MAPPING_MENU_OPTION WHERE REGION_CD='" + region_Id + "' ORDER BY SEQUENCE", null);

            if (dbcursor != null) {
                dbcursor.moveToFirst();
                while (!dbcursor.isAfterLast()) {
                    MappingMenuOptionGetterSetter pgs = new MappingMenuOptionGetterSetter();

                    pgs.setRegion_Id(dbcursor.getString(dbcursor.getColumnIndexOrThrow("REGION_CD")));
                    pgs.setMenu_name(dbcursor.getString(dbcursor.getColumnIndexOrThrow("MENU_NAME")));
                    pgs.setMenu_icon(dbcursor.getString(dbcursor.getColumnIndexOrThrow("MENU_ICON")));
                    pgs.setMenu_icon_done(dbcursor.getString(dbcursor.getColumnIndexOrThrow("MENU_ICON_DONE")));
                    pgs.setMenu_icon_gray(dbcursor.getString(dbcursor.getColumnIndexOrThrow("MENU_ICON_GREY")));
                    pgs.setImage_path(dbcursor.getString(dbcursor.getColumnIndexOrThrow("IMAGE_PATH")));
