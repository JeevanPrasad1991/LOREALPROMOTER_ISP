...
public class UploadAllImageActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Path= Environment.getExternalStorageDirectory() + "/Mondelez_Images/";
					...
					for(int i=0;i<coverageBeanlist.size();i++){
						...
						if (assetInsertdata.size()>0) {
							...
							for (int j = 0; j < assetInsertdata.size(); j++) {
								...
								if (assetInsertdata.get(j).getImg() != null && !assetInsertdata.get(j).getImg().equals("")) {
